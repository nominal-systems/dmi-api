...
export class UnifyProviderOptions1693444089743 implements MigrationInterface {
    ...
    public async up (queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`dmi\`.\`provider_option\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` varchar(255) NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`required\` tinyint NOT NULL, \`providerOptionType\` ENUM ('configuration', 'integration') NOT NULL, \`providerId\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`)
        await queryRunner.query(`ALTER TABLE \`dmi\`.\`provider_option\` ADD CONSTRAINT \`FK_964474fdde11ef40ed5f77a15a3\` FOREIGN KEY (\`providerId\`) REFERENCES \`dmi\`.\`provider\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`)
    }
    ...
    public async down (queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`dmi\`.\`provider_option\` DROP FOREIGN KEY \`FK_964474fdde11ef40ed5f77a15a3\``)
        await queryRunner.query(`DROP TABLE \`dmi\`.\`provider_option\``)
    }
