...
import { FeatureFlagContext, FeatureFlagProvider } from './feature-flag.interface'
...
export class StatsigFeatureFlagProvider implements FeatureFlagProvider, OnModuleInit {
  ...
  isEnabled(flag: string, context?: FeatureFlagContext): boolean {
    ...
    const resolvedIntegrationId =
      ...
      this.getStringValue(custom.integrationId as unknown)
    ...
    this.logger.debug(
      `Flag "${flag}" for integrationId=${resolvedIntegrationId}: ${result}`
    )
